...
export const useParagraphStore = defineStore("paragraph", () => {
  ...
  //delete a content paragraph for a specific blog
  const deleteBlogContentParagraph = (paragraphId: number | string, blogId: number | string) => {
    return new Promise((resolve, reject) => {
      const url = `${apiUrl}/blogs/${blogId}/content/${paragraphId}`;
      //add an access token to the request to access the protected route
      setAuthToken();
      axios
        .delete(url)
